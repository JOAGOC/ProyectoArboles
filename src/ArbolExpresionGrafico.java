...
public class ArbolExpresionGrafico extends JPanel implements ArbolListener 
{
    private ArbolBinarioEntero arbolBinario;
     ...
     * dibujar.
     */
    public ArbolExpresionGrafico(ArbolBinarioEntero miArbol) 
    {
          this.arbolBinario = miArbol;
          this.setBackground(Color.WHITE);
          posicionNodos = new HashMap();
          subtreeSizes = new HashMap();
          arbolBinario.setAl(this);
    ...
    private void calcularPosiciones() 
    {
         posicionNodos.clear();
         subtreeSizes.clear();
         Nodo raiz = arbolBinario.getRaiz();
         if (raiz != null) 
         {
    ...
    private Dimension calcularTamañoSubarbol(Nodo n) 
    {
          ...
          Dimension dimI = calcularTamañoSubarbol(n.getIzq());
          Dimension dimD = calcularTamañoSubarbol(n.getDer());
          int h = fm.getHeight() + parent2child + Math.max(dimI.height, dimD.height);
          int w = dimI.width + child2child + dimD.width;
    ...
    private void dibujarArbol(Graphics2D g, Nodo n, int puntox, int puntoy, int yoffs) 
    {
     ...
     g.draw(r);
     g.drawString(n.getInfo()+"", r.x + 3, r.y + yoffs);
   ...
   public void paint(Graphics g) 
   {
         ...
         dibujarArbol(g2d, arbolBinario.getRaiz(), Integer.MAX_VALUE, Integer.MAX_VALUE, 
